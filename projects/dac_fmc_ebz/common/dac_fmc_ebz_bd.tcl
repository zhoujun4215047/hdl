...
ad_ip_instance util_upack dac_upack [list \
...
ad_connect tx_device_clk dac_upack/dac_clk
...
for {set i 0} {$i < $NUM_OF_CHANNELS} {incr i} {
  ...
  ad_ip_instance xlslice dac_valid_slice_$i [list \
    DIN_WIDTH $NUM_OF_CHANNELS \
    DIN_FROM $i \
    DIN_TO $i \
  ]
  ...
  ad_connect dac_jesd204_transport/dac_valid dac_valid_slice_$i/Din

  ad_connect dac_enable_slice_$i/Dout dac_upack/dac_enable_$i
  ad_connect dac_valid_slice_$i/Dout dac_upack/dac_valid_$i
  ad_connect dac_upack/dac_data_$i dac_data_concat/In$i
}
...
ad_connect dac_upack/dac_valid axi_dac_fifo/dac_valid
ad_connect dac_upack/dac_data axi_dac_fifo/dac_data
